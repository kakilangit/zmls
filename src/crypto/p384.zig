...
// KDF:       HKDF-SHA384       (Hkdf over HmacSha384)
// AEAD:      AES-256-GCM       (std.crypto.aead.aes_gcm.Aes256Gcm)
// DH/KEM:    P-384 ECDH        (std.crypto.ecc.P384)
// Signature: ECDSA-P384-SHA384 (std.crypto.sign.ecdsa.EcdsaP384Sha384)
//
...
const Hkdf = crypto.kdf.hkdf.Hkdf(HmacSha384);
...
pub const DhKemP384Sha384Aes256GcmP384 = struct {
    ...
    /// Hash output length (SHA-384 = 48 bytes).
    pub const nh: u32 = Sha384.digest_length;

    /// AEAD key length (AES-256-GCM = 32 bytes).
    pub const nk: u32 = Aes256Gcm.key_length;

    /// AEAD nonce length (AES-256-GCM = 12 bytes).
    pub const nn: u32 = Aes256Gcm.nonce_length;

    /// AEAD authentication tag length (16 bytes).
    pub const nt: u32 = Aes256Gcm.tag_length;

    /// DH public key length (P-384 uncompressed = 97 bytes).
    pub const npk: u32 = 97;

    /// DH secret key length (P-384 scalar = 48 bytes).
    pub const nsk: u32 = 48;

    /// Signature public key length (P-384 uncompressed = 97).
    pub const sign_pk_len: u32 = 97;

    /// Signature secret key length (P-384 scalar = 48 bytes).
    pub const sign_sk_len: u32 = 48;
    ...
    /// P-384 ECDH: compute shared secret (x-coordinate).
    pub fn dh(
        sk: *const [nsk]u8,
        pk: *const [npk]u8,
    ) CryptoError![48]u8 {
        const peer = P384.fromSec1(pk) catch {
            return error.InvalidPublicKey;
        };
    ...
    comptime {
        provider.assertValid(DhKemP384Sha384Aes256GcmP384);
    }
};
...
test "suite 0x0007 constants match RFC 9420" {
    ...
    try testing.expectEqual(@as(u32, 97), P.sign_pk_len);
    try testing.expectEqual(@as(u32, 48), P.sign_sk_len);
