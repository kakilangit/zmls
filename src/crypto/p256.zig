//! CryptoProvider for MLS cipher suite 0x0002
//! (P-256 ECDH + AES-128-GCM + SHA-256 + ECDSA-P256).
// CryptoProvider backend for MLS cipher suite 0x0002:
...
/// Cipher suite 0x0002 backend.
pub const DhKemP256Sha256Aes128GcmP256 = struct {
    ...
    /// Validate a P-256 public key (uncompressed SEC-1).
    ///
    /// Rejects the identity element and malformed points.
    /// P-256 has cofactor 1 so all non-identity points on the
    /// curve are in the prime-order subgroup.
    ...
    /// Generate a P-256 DH key pair from a 32-byte seed.
    ///
    /// Uses ECDSA's deterministic scalar derivation (RFC 6979-
    ...
    comptime {
        provider.assertValid(DhKemP256Sha256Aes128GcmP256);
    }
};
...
const P = DhKemP256Sha256Aes128GcmP256;

test "p256 aead seal/open round-trip" {
...
test "p256 aead rejects tampered ciphertext" {
    const key = [_]u8{0x42} ** P.nk;
    const nonce = [_]u8{0x24} ** P.nn;
...
test "suite 0x0002 constants match RFC 9420" {
...
test "p256 sign/verify rejects wrong message" {
...
test "p256 validate rejects invalid public key" {
    // All-zeros is not a valid SEC-1 point.
...
// -- Full group lifecycle test with suite 0x0002 --
...
const suite_0x0002: CipherSuite =
    .mls_128_dhkemp256_aes128gcm_sha256_p256;
...
fn makeLeaf0x0002(
    ...
    const suites = comptime [_]CipherSuite{suite_0x0002};
...
const TestKP0x0002 = struct {
    ...
    fn init(
        ...
        self.kp = .{
            .version = .mls10,
            .cipher_suite = suite_0x0002,
            .init_key = &self.init_pk,
            .leaf_node = makeLeaf0x0002(
                &self.enc_pk,
                &self.sign_pk,
            ),
            .extensions = &.{},
...
test "suite 0x0002 full group lifecycle" {
    ...
    // 1. Create group with suite 0x0002.
    var gs = try state_mod.createGroup(
        P,
        alloc,
        "p256-lifecycle",
        makeLeaf0x0002(&alice_enc.pk, &alice_sign.pk),
        suite_0x0002,
        &.{},
    );
    defer gs.deinit();
    ...
    var bob: TestKP0x0002 = undefined;
